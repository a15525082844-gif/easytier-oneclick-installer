...
#!/usr/bin/env bash
[[ -z ${CLI_ARG_LOG:-} ]] || printf '%s\n' "$*" > "$CLI_ARG_LOG"
if [[ ${NODE_MODE:-good} == good ]]; then
  cat <<'JSON'
{"config":"listeners = [\"tcp://0.0.0.0:11010\", \"udp://0.0.0.0:11010\", \"ws://0.0.0.0:11011\"]","listeners":["ring://id","tcp://0.0.0.0:11010","tcp://[::]:11010","udp://0.0.0.0:11010","udp://[::]:11010","ws://0.0.0.0:11011/","ws://[::]:11011/"]}
JSON
...
grep -F -- "-n ${MANAGED_INSTANCE_NAME} node" "$CLI_ARG_LOG" >/dev/null || fail 'health check did not select the managed instance'
((pass+=1))
...
assert_fail json_array_field listeners <<< '{"config":"no runtime field"}'
assert_fail json_array_field listeners <<< '{"listeners":["udp://0.0.0.0:11010"}'
assert_ok listener_present '["ws://0.0.0.0/"]' 'ws://0.0.0.0:80'
assert_ok listener_present '["wss://0.0.0.0/"]' 'wss://0.0.0.0:443'
assert_fail listener_present '["udp://[::]:11010"]' 'udp://0.0.0.0:11010'
