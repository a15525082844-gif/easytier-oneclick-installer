...
readonly MANAGED_INSTANCE_NAME="easytier-oneclick"
...
json_array_field() {
  ...
  awk -v field="$field" '
    ...
    END {
      key = "\"" field "\""
      length_of_document = length(document)
      in_string = 0
      escaped = 0

      for (i = 1; i <= length_of_document; i++) {
        character = substr(document, i, 1)
        if (in_string) {
        ...
        if (substr(document, i, length(key)) != key) {
          in_string = 1
          continue
        }

        position = i + length(key)
        while (position <= length_of_document && substr(document, position, 1) ~ /[[:space:]]/) position++
        ...
        array_string = 0
        array_escaped = 0
        for (j = array_start; j <= length_of_document; j++) {
          array_character = substr(document, j, 1)
          if (array_string) {
            if (array_escaped) array_escaped = 0
            else if (array_character == "\\") array_escaped = 1
            else if (array_character == "\"") array_string = 0
            continue
          }
          if (array_character == "\"") array_string = 1
          else if (array_character == "[") depth++
          else if (array_character == "]") {
            depth--
            if (depth == 0) {
              result = substr(document, array_start, j - array_start)
              found = 1
              break
            }
          }
